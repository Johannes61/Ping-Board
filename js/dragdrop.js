// app.js — sticky footer layout, profile card, per-site timers, pause, per-site interval, notifications
...
function init() {
  ...
  $('#wipeBtn').addEventListener('click', () => {
    ...
    renderLists(); renderCards(); // timers empty now
  });
  ...
  $('#addSiteForm').addEventListener('submit', (e) => {
    ...
    const id = uid();
    state.sites.push({ id, name, url, intervalMs: DEFAULT_SITE_INT }); // site-level interval optional
    Storage.save(state);
...
function restartTimerFor(id) {
  const r = runtime[id]; if (r?.timer) { clearInterval(r.timer); r.timer = null; }
  startTimerFor(id);
}

function restartAllTimers() {
  stopAllTimers();
  for (const id of state.activeIds) startTimerFor(id);
}
...
async function singleTick(id) {
  ...
  // Update UI quickly if card is visible
