(function () {
  ...
  function targetFrom(url) {
    try { const u = new URL(ensureProtocol(url)); return `${u.origin}/favicon.ico?cb=${Date.now()}`; }
    catch { return null; }
  }
  function describeTarget(url) {
    const t = targetFrom(url); if (!t) return { origin: '—', url: '—' };
    const u = new URL(t); return { origin: u.origin, url: t };
  }
  async function pingOnce(url) {
    ...
    try {
      ...
      clearTimeout(timer);
      return { ok: true, ms: Math.round(performance.now() - t0), target };
    } catch {
      clearTimeout(timer);
